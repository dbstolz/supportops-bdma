...
Dim boolCDO, intIndex, intLoop, intDepth, intEmailCount, intError, strNL, strErrMsg, strCurrentHost, strLastHost, strTempDir, strFileList, _
 strLookupFilepath, strZipExe, strApplication, strCurrentPath, strLastPath, strFilePaths, strFolderName, strServerName, strTopFolder, strBaseDirectory, _
 arrFileList, arrPath, arrAccountList, objFSO, objFO, objCDO, objCDG, objTS
	...
	For i = UBound(arrFolders) To 0 Step - 1
		...
		For Each strLine in arrAccountList
			If strLine = "" Then
				...
				Exit For	' Each strLine in arrAccountList
			End If	' strLine = ""
' 			Row counter
			intCount = intCount + 1
...
'			Make a log entry if the 'contracts' folder is the first element of the array, then stop looking for the parent
			If arrAccount(0) = "contracts" Then
				DLogMsg DL_Debug_Some, "LookupAccount: For-For-Each: 'contracts' folder found"
				Exit For	' ' Each strLine in arrAccountList
			End If	' arrAccount(0) = "contracts"
...
'				The hostname was found
				...
				DLogMsg DL_Debug_Some, "LookupAccount: For-For-Each-If2: File field 0 is: " & arrAccount(0)
				DLogMsg DL_Debug_Some, "LookupAccount: For-For-Each-If2: File field 2 is: " & arrAccount(2)
				For j = 0 to UBound(arrAccount)
					If arrAccount(j) = "#N/A" Then
						arrAccount(j) = ""
					End If	' arrAccount(j) = "#N/A"
					boolFound = True
				Next	' j = 0 to UBound(arrAccount)
'				PrintArray arrAccount
'				Hostname was found, so exit internal For loop
				Exit For	' Each strLine in arrAccountList
			Else	' arrAccount(0) = arrFolders(i)
				DLogMsg DL_Debug_More, "LookupAccount: For-For-Each-If2-Else: Account not found"
				boolFound = False
			End If	' arrAccount(0) = arrFolders(i)
		Next	' Each strLine in arrAccountList
		If NOT boolFound Then
			DLogMsg DL_Debug_Some, "LookupAccount: For-For-Each-If3: Account not found"
		Else	' NOT boolFound
'			Hostname was found, so exit outer For..Next loop
			Exit For	' Each strLine in arrAccountList
		End If	' NOT boolFound (1)
	Next	' i = UBound(arrFolders) To 0 Step - 1
...
'	 and the second field of the array to "NOT FOUND" to indicate that the hostname wasn't found in the lookup file 
	If NOT boolFound Then
		...
		DLogMsg DL_Debug_Some, "LookupAccount: For-If1: Hostname argument: " & strHostname
		DLogMsg DL_Debug_Some, "LookupAccount: For-If1: Lookup file hostname: " & arrAccount(0)
...
'	The destination folder will be at the same level as the Parent Folder, so we need to recreate the path up to the level above it
	...
	Next	' intIndex = 1 to Ubound(arrParentPath) - 1
...
' Declare variables
' boolPause - Set to True to force a pause after sending the email message. We do this only when the From address is the default from the 'DI_Email_From' parameter
'   value as a work-around to a Contact record locking/contention issue in Salesforce's email-to-case automation
...
' strPath - Variable used to build a new path and host for files in the "contracts" folder
...
Dim boolPause, intAt, intDot, strPath, strHost, strSFCompressedFile, strFileName, strFiles, strSFFrom, strSFTo, strSFSubject, strSFBody, arrFieldList, _
 arrFileList, arrFilePath
	...
	DLogMsg DL_Debug_Some, "SendFiles: Pass is: " & intSFPass

'	Initialize the boolPause variable
	boolPause = False
...
'	If the top-level folder is "contracts", then start one level down for the search (by stripping first 10 characters from the arguments)
	If strSFHost = "contracts" Then
		strPath = Right(strSFPath, Len(strSFPath) - 10)
		strHost = Left(strPath, InStr(strPath, "\") - 1)
		DLogMsg DL_Debug_Some, "SendFiles: If1: Shortened path is: " & strPath
		DLogMsg DL_Debug_Some, "SendFiles: If1: New host is: " & strHost
		arrFieldList = LookupAccount(strPath, strHost)
 	Else	' strSFHost = "contracts"
 		arrFieldList = LookupAccount(strSFPath, strSFHost)
 	End If	' strSFHost = "contracts"
...
'		Add only the filename, which will be the last element of the array we split each file path into
	...
	Next	' Each strFileName in arrFileList
...
'					Use the address from the lookup file
					DLogMsg DL_Debug_Some, "SendFiles: Case-If-If: Using Lookup file From address"
					strSFFrom = arrFieldList(3)
				Else	' intDot > intAt
'					Invalid entry in the lookup file, so use the default From address
					DLogMsg DL_Debug_Some, "SendFiles: Case-If-If-Else: Using default From address"
					strSFFrom = MIGetTaskParam("DI_Email_From")
					boolPause = True
				End If	' intDot > intAt
			Else	' intAt >
'				Invalid entry in the lookup file, so use the default From address
				DLogMsg DL_Debug_Some, "SendFiles: Case-If-Else: Using default From address"
				strSFFrom = MIGetTaskParam("DI_Email_From")
				boolPause = True
			End If	' intAt >
		Case Else	' LCase(strApplication)
			DLogMsg DL_Debug_Some, "SendFiles: Case-Else: Unknown Application: " & strApplication
			strSFFrom = MIGetTaskParam("DI_Email_From")
			boolPause = True
	End Select	' LCase(strApplication)

	DLogMsg DL_Debug_More, "SendFiles: Message from is: " & strSFFrom
	DLogMsg DL_Debug_More, "SendFiles: Message to is: " & strSFTo
	DLogMsg DL_Debug_More, "SendFiles: Subject line is: " & strSFSubject
	DLogMsg DL_Debug_More, "SendFiles: Body is: " & strSFBody
...
'	Send the email
	...
	If boolPause Then
		DLogMsg DL_Debug_Some, "SendFiles: If2: Pausing " & MIGetTaskParam("DI_Email_Pause") & " milliseconds."
		MISleep(CInt(MIGetTaskParam("DI_Email_Pause")))
	End If	' boolPause
...
' DI_Email_Pause is the number of millisends to pause after sending an email using the default email From address (to accomodate record locking issues in Salesforce's email-to-case automation)
...
DLogMsg DL_Debug_Some, "main: DI_Email_Pause parameter is: " & MIGetTaskParam("DI_Email_Pause")
...
'	Pause time between emails using the default From address (in milliseconds)
	If MIGetTaskParam("DI_Email_Pause") = "" Then intError = 1 : strErrMsg = strErrMsg & strNL & "ERROR-DI_Email_Pause parameter has not been set."
	If Not IsNumeric(MIGetTaskParam("DI_Email_Pause")) Then intError = 1 : strErrMsg = strErrMsg & strNL & "ERROR-DI_Email_Pause parameter must be an integer (milliseconds)."
...
'	Log error message if param(s) missing
	If intError > 0 Then DLogMsg DL_Error_Task, "main: Task Param: Error code is: " & CStr(intError)
...
'	Check that external files and objects are accessible
	...
	If Not MIGetTaskParam("DI_SMTP") = "" Then
'		Can a Collaboration Data Objects Message object be created?
'		Use this test instead of (intError = 0) so we can try to create CDO objects with which to send error email
...
'			Save the values and connect the configuration object to the message object
	...
	End If	' Not MIGetTaskParam("DI_SMTP") = ""
...
'		Can a File System Object be created?
		...
		Else	' Not IsObject(objFSO) Or IsNull(objFSO)
...
'			Obtain "safe" path to it
		...
		Else	' objFSO.FileExists(strZipExe)
		...
		End If	' objFSO.FileExists(strZipExe)
	DLogMsg DL_Debug_More, "main: Zip executable found."
	End If	' intError = 0 (3)
...
'		DLogMsg DL_Debug_Some, "main: Current Path is1: " & strCurrentPath
...
'		It exits after the first pass, so the process ends normally when the For loop runs out of files.
		...
		Do	' While False
			DLogMsg DL_Debug_Some, "main: For-Do: Processing file " & arrFileList(intIndex)
...
'			Catch a top-level file and skip it, but notify someone. The Exit Do allows the script to process the rest of the files without crashing.
			If intDepth < 1 Then
				DLogMsg DL_Warning, "main: For-Do-If1: Directory depth: " & CStr(intDepth)
				DLogMsg DL_Warning, "main: For-Do-If1: Top level file found: " & strBaseDirectory & "\" & arrFileList(intIndex)
				Call SendEmail(MIGetTaskParam("DI_Email_From"), MIGetTaskParam("DI_Error_To"), "Non-Fatal error processing files in task " & MITaskname(), "Top level file found: '" & strBaseDirectory & "\" & arrFileList(intIndex) & "' - skipping file.")
				Exit Do	' While False
			End If	' intDepth < 1

'			Catch a mismatch between the DI_Application task parameter and the parent folder name and skip over the file.
'			Modified the code to treat it as a non-fatal error so that other files in this run will be processed,
'			 but it should be reported since it reveals a disconnect between the current directory depth and the expected parent directory name.
			If arrPath(intDepth - 1) <> strApplication Then
				DLogMsg DL_Warning, "main: For-Do-If2: Directory depth: " & CStr(intDepth)
				DLogMsg DL_Warning, "main: For-Do-If2: Parent directory: " & arrPath(intDepth - 1)
				intError = -1
				strErrMsg = "ERROR-File '" & strBaseDirectory & "\" & arrFileList(intIndex) & "' not found inside the '" & strApplication & "' directory - skipping file." 
				DLogMsg DL_Warning, "main: For-Do-If2: " & strErrMsg
				Call SendEmail(MIGetTaskParam("DI_Email_From"), MIGetTaskParam("DI_Error_To"), "Non-Fatal error processing files in task " & MITaskname(), strErrMsg)
				DLogMsg DL_Warning, "main: For-Do-If2: Skipping file"
'				Set error condition so task aborts
'				MISetErrorDescription(strErrMsg)
'				MISetErrorCode(intError)
				Exit Do	' While False		
			End If	' arrPath(intDepth - 1) <> strApplication

'			Intialize variables for the loop
'			When'contracts' is the top-level folder, set the BaseLevel variable to the second array element (zero-indexed)
			strCurrentHost = arrPath(0)
			DLogMsg DL_Debug_Some, "main: For-Do: Current Host is2: " & strCurrentHost
'			This next For..Next loop builds the path up to the parent directory in which the file is found
			...
			For intLoop = 1 to intDepth - 1
				...
				DLogMsg DL_Debug_More, "main: For-Do-For: Current Path is2: " & strCurrentPath
			Next	' intLoop = 1 to intDepth - 1
			If strLastHost = "" Then
...
'				It only needs to be done on the first pass, after that the loop sets it to the current host only when that changes.
				DLogMsg DL_Debug_Some, "main: For-Do: First pass."
...
'				Use the value from the DI_Application task paramter for now
			...
			End If	' strLastHost = ""
			DLogMsg DL_Debug_More, "main: For-Do: Current Path is3: " & strCurrentPath
...
'			If the current path equals the last path, we're in the same directory, so append the file to the list
			If strCurrentPath = strLastPath Then
				...
				strLastPath = strCurrentPath
				DLogMsg DL_Debug_Some, "main: For-Do-If4: Filepath is: " & strFilePaths
			Else	' strCurrentPath = strLastPath
'				The path has changed, so compress and send the files in the list before moving on
				DLogMsg DL_Debug_Some, "main: For-Do-If4-Else: NEW PATH - Process filelist and send email."
...
'				Prepare for next set of files by setting the current host to the first element of the array
				strCurrentHost = arrPath(0)
'				Now build the path up to the directory in which the file is found
				strCurrentPath = strCurrentHost
				For intLoop = 1 to intDepth - 1
					strCurrentPath = strCurrentPath & "\" & arrPath(intLoop)
					DLogMsg DL_Debug_More, "main: For-Do-If4-Else-For: Current Path is now2: " & strCurrentPath
				Next	' intLoop = 1 to intDepth - 1
...
'				Use the value from the DI_Application task paramter for now
				...
				DLogMsg DL_Debug_More, "main: For-Do-If4-Else: Current Path is4: " & strCurrentPath
				DLogMsg DL_Debug_More, "main: For-Do-If4-Else: Current Host is3: " & strCurrentHost
				strLastHost = strCurrentHost
				DLogMsg DL_Debug_More, "main: For-Do-If4-Else: Current Path is5: " & strCurrentPath
'				DLogMsg DL_Debug_Some, "main: For-Do-If4-Else: Last host is4: " & strLastHost
			End If	' strCurrentPath = strLastPath
		Loop While False

'		If a fatal error condition was set inside the loop, stop processing files and exit the For..Next loop
		If intError > 0 Then Exit For	' intIndex = 0 to UBound(arrFileList)

		Next	' intIndex = 0 to UBound(arrFileList)

'		If a fatal error condition does not exist after exiting the For..Next loop normally, compress and send the final set of files
		If intError <= 0 Then 
'			Remove the trailing | character from the file path
			If Right(strFilePaths,1) = "|" Then strFilePaths = Left(strFilePaths, Len(strFilePaths) - 1)
			Call SendFiles(strFilePaths, strLastPath, strLastHost, intIndex)
...
If intEmailCount > 0 Then
	DLogMsg DL_OK_Task, "main: " & CStr(intEmailCount) & " EMAILS SENT."
'	Call SendEmail(MIGetTaskParam("DI_Email_From"), MIGetTaskParam("DI_Error_To"), MITaskname() & " sent " & CStr(intEmailCount) & " email(s)", "")
End If	' intEmailCount >
